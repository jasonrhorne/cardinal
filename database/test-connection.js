...
async function validateExtensions() {
  ...
  try {
    ...
    if (missingExtensions.length > 0) {
      console.error('❌ Missing required extensions:', missingExtensions.join(', '));
      return false;
    }
    
    console.log('✅ All required extensions are installed:', installedExtensions.join(', '));
    return true;
  } catch (error) {
...
async function validateMigrations() {
  ...
  try {
    ...
    const missingMigrations = expectedMigrations.filter(migration => !appliedMigrations.includes(migration));
    
    if (missingMigrations.length > 0) {
      console.error('❌ Missing migration records:', missingMigrations.join(', '));
      return false;
    }
...
async function testRlsPolicies() {
  ...
  try {
    ...
    if (!usersError) {
      throw new Error('RLS not working - anonymous access to users table succeeded');
    }
...
async function generateStats() {
  ...
  try {
    ...
    console.log('   --------------------------------');
    data.forEach(row => {
      console.log(`   ${row.tablename.padEnd(12)} | ${row.size.padEnd(8)} | ${row.inserts.toString().padEnd(7)} | ${row.updates.toString().padEnd(7)} | ${row.deletes}`);
    });
